...
describe('la partie — ce qu’un joueur peut changer', () => {
  ...
  it('après « Rejouer », les équipes changent avant que tout le monde soit revenu', async () => {
    ...
    await assertSucceeds(updateDoc(game(as(UID.romain)), { eventSeq: 6, seating: autre, dealer: autre[1] }))
    ...
    await assertFails(updateDoc(game(as(UID.romain)), { eventSeq: 6, seating: intrus, dealer: intrus[1] }))
  })
})
