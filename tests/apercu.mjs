...
for (const taille of TAILLES) {
  ...
  await benel.getByRole('button', { name: 'Benel' }).click()
  await benel.getByRole('button', { name: 'Créer une nouvelle partie' }).click()
  await benel.waitForSelector('text=Autour de la table', { timeout: 20000 })
  const code = (await benel.locator('.font-display').first().innerText()).trim()
