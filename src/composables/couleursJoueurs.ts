 ...
 * `joueurs` : moi, mon partenaire, puis les deux adversaires.
 */
export function couleursPartie(joueurs: readonly PlayerId[]): Record<PlayerId, string> {
  const tons = ['#d9a441', '#e0876a', '#7fa8c9', '#8fbf7a']
  return Object.fromEntries(joueurs.map((p, i) => [p, tons[i] ?? PALETTE[i % PALETTE.length]]))
}
