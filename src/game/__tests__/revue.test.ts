...
function donneJouee(numero: number, seq0: number): { events: GameEvent[]; mains: Record<PlayerId, Card[]> } {
  ...
  const events: GameEvent[] = [
    ...
    ev({ type: 'contrat_fixe', taker: 'viv', value: 80, trump: 's', declaration: 's', multiplier: 1, capot: false, generale: false }),
  ]
  ...
  while (etat.completed.length < 8) {
    const qui = etat.current.length === 0
    ...
    events.push(ev({ type: 'carte_jouee', player: qui, card: carte, trickNumber: avant + 1, position: etat.current.length }))
