...
describe('ce que chacun sait des autres', () => {
  it('les cartes jugées impossibles ne sont jamais dans la vraie main, sur 300 donnes', () => {
    for (let graine = 1; graine <= 300; graine++) {
      ...
      const plis = d.plis.map((p) =>
        p.cartes.map((c) => ({ siege: seatOf(c.player, DEFAULT_SEATING), carte: c.card })),
      )
      ...
      for (let k = 1; k <= 8; k++) {
        ...
        for (const p of DEFAULT_SEATING) {
          ...
          for (const c of reste)
            expect(interdit[s].has(c), `graine ${graine}, pli ${k}, ${p} a ${c}`).toBe(false)
        }
      }
    }
  })
...
describe('chaque carte jugée', () => {
  it("juge toutes les cartes d'une donne, et mesure le temps", () => {
    ...
    for (const n of [8, 20]) {
      ...
      console.log(
        `${n} tirages par carte : ${Date.now() - debut} ms ; ` +
          jugees
            .map((j) => `${j.carte}:${j.qualite}${j.perteChances ? `(-${j.perteChances.toFixed(0)}%)` : ''}`)
            .join(' '),
      )
  ...
  it('découpé en morceaux (un par fil de calcul), le résultat est identique', () => {
    ...
    const pairs = analyserCartes(d, DEFAULT_SEATING, {
      echantillons: 4,
      graine: 9,
      seulement: [...Array(32).keys()].filter((i) => i % 2 === 0),
    })
    const impairs = analyserCartes(d, DEFAULT_SEATING, {
      echantillons: 4,
      graine: 9,
      seulement: [...Array(32).keys()].filter((i) => i % 2 === 1),
    })
    const recolle = [...pairs, ...impairs].sort(
      (a, b) =>
        a.pli - b.pli ||
        entier.indexOf(entier.find((x) => x.carte === a.carte)!) -
          entier.indexOf(entier.find((x) => x.carte === b.carte)!),
    )
