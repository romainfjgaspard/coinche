...
const pass = (player: 'benel' | 'roux' | 'viv' | 'romain'): BiddingEntry => ({ kind: 'pass', player })
...
describe('ENC-1 — ordre de parole', () => {
  ...
  it('puis on tourne dans le sens du jeu', () => {
    expect(currentBidder(run(pass('viv')))).toBe('roux')
    expect(currentBidder(run(pass('viv'), pass('roux')))).toBe('romain')
    expect(currentBidder(run(pass('viv'), pass('roux'), pass('romain')))).toBe('benel')
  })

  it('refuse une prise de parole hors tour', () => {
    expect(() => apply(start(), pass('roux'))).toThrow(IllegalBid)
  })
})
...
describe('ENC-3 / ENC-4 — paliers et surenchère', () => {
  ...
  it("n'autorise plus que le dessus d'une enchère existante", () => {
    const s = run({ kind: 'contract', player: 'viv', value: 90, suit: 'h' })
    expect(legalValues(s)).toEqual([100, 110, 120, 130, 140, 150, 160, 170])
  })

  it('refuse une enchère égale ou inférieure', () => {
    const s = run({ kind: 'contract', player: 'viv', value: 90, suit: 'h' })
    expect(() => apply(s, { kind: 'contract', player: 'roux', value: 90, suit: 's' })).toThrow(IllegalBid)
    expect(() => apply(s, { kind: 'contract', player: 'roux', value: 80, suit: 's' })).toThrow(IllegalBid)
  })
})
...
describe('ENC-5 — reparler après avoir passé', () => {
  it('un joueur qui a passé peut annoncer au tour suivant', () => {
    const s = run(
      pass('viv'),
      { kind: 'contract', player: 'roux', value: 80, suit: 'h' },
      pass('romain'),
      pass('benel'),
    )
    expect(currentBidder(s)).toBe('viv')
    const after = apply(s, { kind: 'contract', player: 'viv', value: 90, suit: 's' })
...
describe('ENC-6 / ENC-7 — fin des enchères', () => {
  it('trois passes après une enchère closent les enchères', () => {
    const s = run({ kind: 'contract', player: 'viv', value: 100, suit: 'h' }, pass('roux'), pass('romain'))
    expect(outcome(s).status).toBe('open')
    const closed = apply(s, pass('benel'))
    expect(outcome(closed)).toMatchObject({ status: 'contract', taker: 'viv', value: 100, trump: 'h' })
    expect(currentBidder(closed)).toBeNull()
  })

  it("quatre passes d'emblée donnent une donne blanche", () => {
    const s = run(pass('viv'), pass('roux'), pass('romain'), pass('benel'))
    expect(outcome(s).status).toBe('passed_out')
  })
})
...
describe('ENC-8 / ENC-9 / ENC-10 — capot, générale, sans-atout', () => {
  it('le capot passe au-dessus de tout contrat chiffré et vaut 250', () => {
    const s = run(
      { kind: 'contract', player: 'viv', value: 160, suit: 'h' },
      { kind: 'capot', player: 'roux', declaration: 's' },
      pass('romain'),
      pass('benel'),
      pass('viv'),
    )
    expect(outcome(s)).toMatchObject({ status: 'contract', taker: 'roux', value: 250, capot: true })
  })
  ...
  it("sans-atout et tout-atout se jouent en capot, sans couleur d'atout", () => {
    ...
    expect(outcome(s)).toMatchObject({ status: 'contract', declaration: 'sa', trump: null, capot: true })
  })
})

describe('CO-1 à CO-3 — coinche', () => {
  const taken = () => run({ kind: 'contract', player: 'viv', value: 100, suit: 'h' })
  ...
  it("seul le camp du preneur peut surcoincher, et l'enjeu est quadruplé", () => {
    ...
    const surcoinched = apply(s, { kind: 'surcoinche', player: 'romain' })
    expect(multiplier(surcoinched)).toBe(4)
    expect(outcome(surcoinched)).toMatchObject({ status: 'contract', multiplier: 4, taker: 'viv' })
  })

  it('après une coinche, plus personne ne surenchérit', () => {
    const s = apply(taken(), { kind: 'coinche', player: 'roux' })
    expect(() => apply(s, { kind: 'contract', player: 'benel', value: 110, suit: 's' })).toThrow(IllegalBid)
  })
  ...
  it('CO-5 — même le preneur, à qui revient la parole, ne peut que passer ou surcoincher', () => {
    ...
    expect(() => apply(s, { kind: 'contract', player: 'viv', value: 110, suit: 'h' })).toThrow(IllegalBid)
  ...
  it("le preneur peut laisser passer la coinche : l'enjeu reste doublé", () => {
    const s = apply(taken(), { kind: 'coinche', player: 'roux' })
    expect(currentBidder(s)).toBe('viv')
    const closed = apply(s, pass('viv'))
    expect(outcome(closed)).toMatchObject({ status: 'contract', multiplier: 2 })
  })
})
...
describe('JEU-1 / ENC-9 — qui entame', () => {
  it('normalement le joueur à gauche du donneur', () => {
    const s = run(
      pass('viv'),
      { kind: 'contract', player: 'roux', value: 90, suit: 'h' },
      pass('romain'),
      pass('benel'),
      pass('viv'),
    )
