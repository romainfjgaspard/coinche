...
describe('au-delà des quatre du départ', () => {
    ...
    partie({
      ...
      joueurs: {
        jean: {
          ...
          chutes: 1,
          offerts: 160,
          detail: [{ deal: 1, value: 80, force: 6, reussi: false, capot: false }],
        },
      },
    }),
  ]
  ...
  it('un duo garde la trace de chaque paire affrontée', () => {
    ...
    expect(rv.contre.map(clePaire).sort()).toEqual(
      [clePaire(['benel', 'jean']), clePaire(['benel', 'roux'])].sort(),
    )
  })
  ...
  it('les paliers et le résumé tiennent compte du nouveau', () => {
    expect(parPalier(AVEC_JEAN, ['jean']).find((l) => l.palier === 80)).toMatchObject({
      reussis: 0,
      chutes: 1,
    })
...
describe('bots regroupés, soirées, enchères et temps', () => {
  it("deux « Bot » d'une même partie n'en font qu'un, avec leurs chiffres additionnés", async () => {
    ...
    const a = partie({
      code: 'BOTS',
      seating: ['romain', 'bot-simple-1', 'bot-etoile-1', 'bot-simple-2'],
      scores: [1000, 500],
      joueurs: {
  ...
  it("additionne rôles, écarts et temps d'une partie à l'autre", async () => {
    ...
    const arch = [
      partie({
        ...
        joueurs: {
          romain: {
            roles: { lanceur: 1, suiveur: 0, seul: 2 },
            ecarts: { reussis: [10], chutes: [] },
            temps: { encheres: [1.2], cartes: [3] },
            annonces: { '90': 1 },
          },
        },
      }),
      partie({
        ...
        joueurs: {
          romain: {
            roles: { lanceur: 2, suiveur: 1, seul: 0 },
            ecarts: { reussis: [], chutes: [-20] },
            temps: { encheres: [4], cartes: [] },
            annonces: { '90': 2, passe: 1 },
          },
        },
      }),
    ]
