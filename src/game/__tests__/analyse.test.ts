...
function donneAuHasard(graine: number, valeur = 80) {
  ...
  const events: GameEvent[] = [
    ...
    ev({ type: 'contrat_fixe', taker: 'viv', value: valeur, trump: 's', declaration: 's', multiplier: 1, capot: false, generale: false }),
  ]
...
describe('aurait-on pu gagner ?', () => {
  ...
  it('refuse poliment ce qu\'il ne sait pas analyser', () => {
    ...
    expect(analyserDonne({ ...d, contrat: { ...d.contrat!, generale: true } }, DEFAULT_SEATING).impossible).toMatch(/générale/)
  })
})
