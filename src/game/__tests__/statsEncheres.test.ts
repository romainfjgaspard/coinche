...
import {
  BORNES_TEMPS, ajouterRepartition, ecartsAnnonce, enchereMoyenneDe, histogramme, labelTemps,
  repartitionAnnonces, rolesPrise, tempsParJoueur,
} from '../statsEncheres'
...
const dit = (entry: BiddingEntry, thinkMs?: number) =>
  ev({ type: 'enchere', player: entry.player, round: 1, entry, ...(thinkMs !== undefined ? { thinkMs } : {}) })
const passe = (p: PlayerId) => dit({ kind: 'passe', player: p })
const annonce = (p: PlayerId, value: number, suit: 's' | 'h' | 'd' | 'c') => dit({ kind: 'contrat', player: p, value, suit })
const contrat = (taker: PlayerId, value: number, suit: 's' | 'h' | 'd' | 'c', extra: Record<string, unknown> = {}) =>
  ev({ type: 'contrat_fixe', taker, value, trump: suit, declaration: suit, multiplier: 1, capot: false, generale: false, ...extra })
const fin = (status: string, compared: [number, number], extra: Record<string, unknown> = {}) =>
  ev({ type: 'donne_terminee', dealNumber: 0, status, cardPoints: compared, compared, scores: [0, 0], beloteDeclaredBy: null, beloteForgottenBy: null, etoile: null, ...extra })

describe('lanceur, suiveur ou seul', () => {
  it('A 80♠, B 90♠, A 100♠ : A lance, B suit — les relances n\'y changent rien', () => {
    const r = rolesPrise([
      donne(1), annonce('romain', 80, 's'), passe('benel'), annonce('viv', 90, 's'), passe('roux'),
      annonce('romain', 100, 's'), passe('benel'), passe('viv'), passe('roux'), contrat('romain', 100, 's'),
    ], DEFAULT_SEATING)
  ...
  it('le partenaire annonce une autre couleur : le preneur est seul dans la sienne', () => {
    const r = rolesPrise([
      donne(1), annonce('romain', 80, 'h'), passe('benel'), annonce('viv', 90, 's'), passe('roux'),
      passe('romain'), passe('benel'), passe('viv'), contrat('viv', 90, 's'),
    ], DEFAULT_SEATING)
  ...
  it('la couleur ouverte par un adversaire ne compte pas : on regarde son équipe', () => {
    const r = rolesPrise([
      donne(1), passe('romain'), annonce('benel', 80, 's'), annonce('viv', 90, 's'), passe('roux'),
      annonce('romain', 100, 's'), passe('benel'), passe('viv'), passe('roux'), contrat('romain', 100, 's'),
    ], DEFAULT_SEATING)
...
describe('répartition des annonces', () => {
  it('compte chaque prise de parole, passes comprises', () => {
    const r = repartitionAnnonces([
      donne(1), annonce('romain', 80, 's'), passe('benel'), annonce('viv', 90, 's'), passe('roux'),
      annonce('romain', 100, 's'), passe('benel'), passe('viv'), passe('roux'),
...
describe('écart à l\'annonce', () => {
  it('points faits moins l\'annonce, séparés entre réussis et chutés ; capot à part', () => {
    const r = ecartsAnnonce([
      donne(1), contrat('romain', 100, 's'), fin('reussi', [112, 50]),
      donne(2), contrat('benel', 90, 'h'), fin('chute', [120, 42]),
      donne(3), contrat('romain', 80, 'd'), fin('reussi', [95, 67]),
      donne(4), contrat('romain', 250, 's', { capot: true }), fin('capot', [252, 0]),
    ], DEFAULT_SEATING)
