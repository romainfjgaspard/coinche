...
describe('donne complète', () => {
  ...
  function playOut(trump: 's' | 'h' | 'd' | 'c', firstLeader: PlayerId) {
    const remaining: Record<string, Card[]> = Object.fromEntries(
      PLAYER_IDS.map((p) => [p, [...hands[p]]]),
    )
  ...
  it('et totalise toujours 162 points', () => {
    ...
    const contract: Contract = {
      takerSeat: 2, value: 100, trump: 's', multiplier: 1, capot: false, generale: false,
    }
...
describe('DEC-8 / DEC-9 — étoiles et honte complète', () => {
  const dealWithStar = (player: PlayerId | null): GameEvent =>
    ({
      type: 'donne_terminee', dealNumber: 1, status: 'reussi',
      cardPoints: [162, 0], compared: [162, 0], scores: [100, 0],
      beloteDeclaredBy: null, beloteForgottenBy: null, etoile: player, seq: 0, at: 0,
    }) as GameEvent
...
describe('découpage par donne', () => {
  ...
  it('ne garde que ce qui suit la dernière distribution', () => {
    const journal = [
      ...
      ev('enchere'), ev('carte_jouee'), ev('donne_terminee'),
  ...
  it('une belote annoncée à la donne précédente ne compte plus', () => {
    const journal = [
      ev('donne_commencee', { dealNumber: 1 }),
      ev('belote_annoncee', { player: 'viv' }), ev('belote_annoncee', { player: 'viv' }),
      ev('donne_commencee', { dealNumber: 2 }),
    ]
