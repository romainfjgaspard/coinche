...
import { DERNIER_CODE_KEY } from '../stores/session'
 ...
 */
function codeInitial(): string {
  const lien = new URLSearchParams(location.search).get('code')?.trim().toUpperCase() ?? ''
  if (/^[A-Z]{4}$/.test(lien)) {
  ...
  try { return localStorage.getItem(DERNIER_CODE_KEY) ?? '' } catch { return '' }
}

/** Retirer un nom de la liste de ce navigateur ; ses parties restent en base. */
function retirer(p: PlayerId): void {
  roster.masquer(p)
...
let prerempli = code.value !== ''
watch(cleanCode, async (value) => {
  const deMemoire = prerempli
  prerempli = false
  if (value.length !== 4) return
  await session.peek(value)
  if (deMemoire && !session.game && cleanCode.value === value) {
    code.value = ''
    session.error = null
  }
}, { immediate: true })
...
const regardee = computed(() => cleanCode.value.length === 4 && session.game !== null && !partieFinie.value)
/** Un code pré-rempli peut viser une partie finie : elle ne doit griser personne. */
const partieFinie = computed(() =>
  cleanCode.value.length === 4 && (session.game?.phase === 'terminee' || session.game?.phase === 'annulee'),
)
const canJoin = computed(() => Boolean(chosen.value) && cleanCode.value.length === 4 && !partieFinie.value)
...
<template>
  ...
  >
    ...
    <h2 class="mt-9 mb-4 lg:mt-12 [@media(max-height:820px)]:mt-7 text-[15px] font-semibold text-mist">Qui es-tu ?</h2>
    ...
    <div class="flex flex-col gap-3">
      ...
      -->
      <div class="-mr-1.5 flex max-h-[19.25rem] flex-col gap-3 overflow-y-auto pr-1.5 [@media(max-height:820px)]:max-h-[15rem] [scrollbar-color:rgba(255,255,255,.18)_transparent] [scrollbar-width:thin]">
        <div v-for="p in roster.joueurs" :key="p" class="relative flex shrink-0">
          <button
            type="button"
          ...
          <button
            ...
            :aria-label="`Retirer ${nomDe(p)} de la liste`"
            :title="`Retirer ${nomDe(p)} de cette liste (rien n'est effacé)`"
            @click="retirer(p)"
          >✕</button>
        </div>
      </div>
      ...
      <p v-else-if="roster.info" class="text-center text-[13px] leading-snug text-sage">{{ roster.info }}</p>
    </div>

    <div class="grow lg:hidden"></div>

    <label for="code" class="mt-7 mb-2 lg:mt-10 [@media(max-height:820px)]:mt-6 block text-[13px] text-sage">Code de la partie</label>
    ...
    <p v-if="partieFinie" class="mt-1.5 text-xs text-sage">
      La partie {{ cleanCode }} est {{ session.game?.phase === 'annulee' ? 'annulée' : 'terminée' }} : crée-en une nouvelle.
    </p>
