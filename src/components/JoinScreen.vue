...
function codeInitial(): string {
  ...
  try {
    return localStorage.getItem(DERNIER_CODE_KEY) ?? ''
  } catch {
    return ''
  }
}
    ...
    >
      ...
      <div class="flex gap-2.5">
        ...
        <button
          ...
          @click="rejoindre"
        >
          {{ enCours === 'rejoindre' ? '…' : 'Rejoindre' }}
        </button>
      </div>
      <p v-if="partieFinie" class="mt-1.5 text-xs text-sage">
        La partie {{ cleanCode }} est {{ session.game?.phase === 'annulee' ? 'annulée' : 'terminée' }} :
      ...
      <button
        ...
        @click="creer"
      >
        {{ enCours === 'creer' ? 'Création de la partie…' : 'Créer une nouvelle partie' }}
      </button>
