...
const donnes = computed(() =>
  props.events
    ...
    .map((e) => (e.type === 'donne_commencee' ? revoirDonne(props.events, e.dealNumber, props.seating) : null))
    .filter((d): d is NonNullable<typeof d> =>
      Boolean(d && d.mains && d.contrat && !d.blitz && !d.contrat.generale
...
const SYMBOLE: Record<Qualite, string> = { meilleure: '!', bonne: '✓', imprecision: '?!', erreur: '?', gaffe: '??', forcee: '—' }
const COULEUR: Record<Qualite, string> = {
  meilleure: '#52a884', bonne: '#8fbf7a', imprecision: '#e8c86a', erreur: '#e0876a', gaffe: '#e05a48', forcee: '#6b7f75',
}
...
const bilan = computed(() => {
  ...
  return joueurs.value.map((p) => {
    ...
    const compte = Object.fromEntries(ORDRE.map((q) => [q, siennes.filter((j) => j.qualite === q).length])) as Record<Qualite, number>
    return {
      p,
      compte,
  ...
  <section class="mt-4 rounded-xl border border-white/10 bg-white/4 px-4 py-3">
    ...
    <template v-if="!resultats">
      <p class="mt-1 text-xs leading-relaxed text-sage">
        Chaque carte de chaque donne jugée avec ce que le joueur savait, comme aux échecs : qui joue le plus juste,
        et quelles cartes ont coûté le plus cher.
      </p>
      <button
        v-if="!enCours"
        type="button"
        ...
        @click="analyser"
      >{{ donnes.length ? `Analyser les ${donnes.length} donne${donnes.length > 1 ? 's' : ''} · environ ${Math.max(1, Math.round((donnes.length * 15) / 60))} min` : 'Aucune donne à analyser' }}</button>
      ...
      <table class="mt-2 w-full border-collapse text-[13px] tabular-nums">
        <thead>
          <tr class="text-[11px] text-sage">
            ...
            <th v-for="q in ORDRE" :key="q" class="pb-1 text-center font-semibold" :style="{ color: COULEUR[q] }">{{ SYMBOLE[q] }}</th>
        ...
        <tbody>
          <tr v-for="b in bilan" :key="b.p" class="border-t border-white/8">
            ...
            <td v-for="q in ORDRE" :key="q" class="py-1.5 text-center" :class="b.compte[q] ? '' : 'text-dusk'">{{ b.compte[q] }}</td>
            <td class="py-1.5 text-right font-semibold">{{ b.justes === null ? '—' : `${b.justes} %` }}</td>
          </tr>
        </tbody>
      </table>
      <p class="mt-1 text-[11px] text-dusk">! meilleure · ✓ bonne · ?! imprécision · ? erreur · ?? gaffe · cartes forcées non comptées.</p>

      <h3 class="mt-3 text-[13px] font-semibold">Les tournants de la partie</h3>
      <p v-if="!tournants.length" class="mt-1 text-xs text-sage">Aucune carte n'a coûté plus de 12 % de chances à son camp.</p>
