...
const heure = (ms: number): string => new Date(ms).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })
...
const equipes = (p: PartieNonFinie): string =>
  p.seating ? `${duo(p.seating[0], p.seating[2])} contre ${duo(p.seating[1], p.seating[3])}` : p.joueurs.map(nomDe).join(', ')
</script>
...
<template>
  <section>
    ...
    <p v-if="!liste.length" class="py-3 text-center text-sm text-sage">Aucune partie terminée pour l'instant.</p>
    <div v-for="s in liste" :key="s.cle" class="border-t border-white/8">
      ...
      >
        ...
        <span class="shrink-0 text-xs text-sage">{{ s.parties.length }} partie{{ s.parties.length > 1 ? 's' : '' }}</span>
        <span class="grow truncate text-right text-xs text-mist">
          {{ s.victoires.filter((v) => v.gagnees > 0).map((v) => `${duo(v.paire[0], v.paire[1])} ${v.gagnees}`).join(' · ') }}
        </span>
      ...
      <div v-if="ouverte === s.cle" class="pb-3">
        <div v-for="a in s.parties" :key="a.code" class="flex items-center gap-3 py-1 pl-3 text-[12px]">
          ...
          <span class="grow truncate">
            <span :class="a.winner === 0 ? 'font-semibold text-ivory' : 'text-mist'">{{ duo(a.seating[0], a.seating[2]) }}</span>
            <span class="text-dusk"> contre </span>
            <span :class="a.winner === 1 ? 'font-semibold text-ivory' : 'text-mist'">{{ duo(a.seating[1], a.seating[3]) }}</span>
          </span>
  ...
  <section>
    ...
    <p class="mb-2 text-xs text-sage">Commencées mais pas allées au bout. Elles ne comptent dans aucune autre statistique.</p>
    <p v-if="nonFinies === null" class="py-3 text-center text-sm text-sage">Chargement…</p>
    <p v-else-if="!nonFinies.length" class="py-3 text-center text-sm text-sage">Aucune : toutes les parties sont allées au bout.</p>
    <div
    ...
    >
      ...
      <span class="grow">
        ...
        <span class="text-dusk">{{ p.donnes }} donne{{ p.donnes > 1 ? 's' : '' }} · {{ p.scores[0] }} – {{ p.scores[1] }}</span>
      </span>
      <span
        class="shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold"
        :class="statut(p) === 'annulée' ? 'bg-red-card/25 text-[#f0a293]' : statut(p) === 'en cours' ? 'bg-gold/20 text-gold' : 'bg-white/10 text-mist'"
      >{{ statut(p) }}</span>
    </div>
  </section>
</template>
