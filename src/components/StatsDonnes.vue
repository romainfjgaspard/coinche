...
const enseigneDe = (x: string | null | undefined): { enseigne: string; rouge: boolean } =>
  !x ? { enseigne: '', rouge: false }
    : x === 'sa' || x === 'ta' ? { enseigne: x.toUpperCase(), rouge: false }
      : { enseigne: SUIT_GLYPH[x as 's' | 'h' | 'd' | 'c'], rouge: x === 'h' || x === 'd' }
const texteDe = (b: BiddingEntry): { texte: string; enseigne: string; rouge: boolean } => {
  switch (b.kind) {
    case 'passe': return { texte: 'Passe', enseigne: '', rouge: false }
    case 'contrat': return { texte: String(b.value), ...enseigneDe(b.suit) }
    case 'capot': return { texte: 'Capot', ...enseigneDe(b.declaration) }
    case 'generale': return { texte: 'Générale', ...enseigneDe(b.declaration) }
    default: return { texte: b.kind, enseigne: '', rouge: false }
  }
}
const ISSUE: Record<string, string> = { reussi: 'réussi', chute: 'chuté', capot: 'capot', generale: 'générale' }
...
const lignes = computed<Ligne[]>(() => {
  ...
  for (const e of props.events) {
    if (e.type === 'donne_commencee') {
      courante = { numero: e.dealNumber, donneur: e.dealer, paroles: [], contrat: null, issue: 'en cours', scores: null }
      out.push(courante)
    } else if (!courante) continue
    else if (e.type === 'enchere') courante.paroles.push({ player: e.player, ...texteDe(e.entry), coinche: false })
    else if (e.type === 'coinche' || e.type === 'surcoinche') {
      courante.paroles.push({
        player: e.player, texte: e.type === 'coinche' ? 'Coinche' : 'Surcoinche', enseigne: '', rouge: false, coinche: true,
      })
...
const ordre = (donneur: PlayerId): PlayerId[] => {
  ...
  return [premier, nextPlayer(premier, props.seating), nextPlayer(nextPlayer(premier, props.seating), props.seating), donneur]
}
const couleur = (p: PlayerId): string => (teamOfPlayer(p, props.seating) === props.nous ? 'text-gold' : 'text-them')
</script>
    ...
    <div v-for="l in lignes" :key="l.numero" class="border-t border-white/8">
      ...
      >
        ...
        <span class="grow truncate text-[13px]">
          <template v-if="l.contrat">
            ...
            <span v-if="l.contrat.multiplicateur > 1" class="text-[#f0a293]"> ×{{ l.contrat.multiplicateur }}</span>
          </template>
        ...
        <span v-if="l.scores" class="w-16 shrink-0 text-right text-[13px] tabular-nums">
          <span class="text-gold">{{ l.scores[nous] }}</span> · <span class="text-them">{{ l.scores[nous === 0 ? 1 : 0] }}</span>
        </span>
        <span class="text-xs text-dusk transition" :class="ouverte === l.numero ? 'rotate-180' : ''">▼</span>
      </button>
      <div v-if="ouverte === l.numero" class="pb-3 pl-12">
        ...
        <div class="grid grid-cols-4 gap-x-2 text-[12px]">
          <span v-for="p in ordre(l.donneur)" :key="p" class="truncate pb-1 font-semibold" :class="couleur(p)">{{ nomDe(p) }}</span>
        </div>
        <div class="grid grid-cols-4 gap-x-2 gap-y-0.5 text-[12px]">
          <span
            ...
            :class="pa.coinche ? 'font-semibold text-[#f0a293]' : pa.texte === 'Passe' ? 'text-sage' : 'font-semibold'"
          >{{ pa.texte }} <span v-if="pa.enseigne" :class="pa.rouge ? 'text-[#e8786a]' : ''">{{ pa.enseigne }}</span></span>
        </div>
        <button
